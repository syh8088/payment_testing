...
//         * 결제 승인 응답이 만약에
//         *
//         * - {"error":{"message":"중복 요청 입니다.","developerMessage":"이미 사용한 transactionId는 다시 사용할 수 없습니다."}}
//         * 응답이 나오면 {@link HyosungCreditCardRequestClient#getPayments(transactionId(효성 거래 ID))} 조회 해서 결제 조회 통해 응답에 따라 다시 결과 처리 할 수도록 합니다.
//         *
//         * 이러한 조치를 취하는 이유는 실질적으로 효성으로 부터 결제 승인을 처리가 완료되었는데 이후
//         * 닥톡 서버 시스템이 장애가 발생되거나 결제 상태 업데이트 처리 하는 과정에서 Exception 발생시 대비 하기 위함
//         * 그래서 해당 {@link com.dfs.doctalk.api.entities.paymentevent.PaymentEvent#paymentKey} 효성 전용 거래 ID 기준으로 다시 결제 호출시에도
//         * 2번 결제승인 처리 하지 않고 상태 업데이트만 처리 하기 위함
//         */
//        if (exception.getMessage().equals("중복 요청 입니다.")) {
//            HyosungGetPaymentResponse hyosungGetPaymentResponse = this.feignGetPayment(paymentExecutorInPut.getPaymentKey());
//            return new HyosungPaymentCreditCardWithPspRawDataResponse(hyosungGetPaymentResponse, pspRawData);
//        }
//
//        return HyosungPaymentCreditCardWithPspRawDataResponse.initDueToError(paymentExecutorInPut, exception, pspRawData);
    }
...
//                = tossPaymentClient.paymentError(request.getOrderId(), "ALREADY_COMPLETED_PAYMENT", request);
        ...
        // 재시도가 가능한 실패
        final ResponseEntity<String> paymentsResponse
                = tossPaymentClient.paymentError(request.getOrderId(), "UNKNOWN_PAYMENT_ERROR", request);
